...
def _build_format_prompt(verified_sources: list[SourceRecord], fmt: str) -> str:
    ...
    for i, s in enumerate(verified_sources, start=1):
        # Используем явные метки вместо N/A — чтобы LLM не заполнял пустоты
        def _f(v) -> str:
    ...
    forbidden = (
        "АБСОЛЮТНЫЕ ЗАПРЕТЫ — нарушение любого пункта делает ответ неверным:\n"
        "❌ Нельзя придумывать или угадывать: год, издание (2-е, 3-е и т.д.), страницы, "
        "DOI, ISBN, EDN, URL, город, издательство, том, номер журнала, соавторов, редакторов.\n"
        "❌ Если поле помечено ——ОТСУТСТВУЕТ——, оставь это место пустым в ссылке. "
        "Не заменяй на тире, не пиши 'б/г', 'б/и', не ставь знак вопроса.\n"
        "❌ Не добавляй в список ни одного источника которого нет в verified_sources.\n"
        "❌ Не переводи названия. Русские — на русском, английские — на английском.\n"
        "❌ Не пиши авторов КАПСЛОКОМ.\n"
        "❌ DOI включай ТОЛЬКО если он явно дан в поле doi= (не ——ОТСУТСТВУЕТ——).\n"
    )

    return (
        f"Оформи список источников строго в формате {fmt}.\n"
        "Твоя задача — ТОЛЬКО расставить знаки препинания и порядок элементов по правилам формата.\n"
        "Все фактические данные берёшь ИСКЛЮЧИТЕЛЬНО из verified_sources ниже.\n\n"
        + forbidden +
        "\nverified_sources:\n" + "\n".join(lines)
    )
...
Твоя задача — исправить ТОЛЬКО знаки препинания, пробелы, порядок элементов и \
курсив (для APA/Vancouver). Больше ничего.

АБСОЛЮТНЫЕ ЗАПРЕТЫ — нарушение любого пункта делает ответ неверным:
❌ Нельзя добавлять данные которых нет: год, номер издания, страницы, том, номер журнала, \
издательство, город, DOI, ISBN, EDN, URL, соавторов, редакторов.
❌ Нельзя удалять или добавлять источники.
❌ Если данных для полного оформления не хватает — оформи с тем что есть, не дополняй из памяти.

Верни только исправленный пронумерованный список. Без пояснений и комментариев.
...
async def generate_sources_by_topic(topic: str, count: int, fmt: str, mode: str = "mixed") -> str:
    ...
    # Шаг 1: детерминированное форматирование по правилам (без LLM — не додумывает)
    formatted = format_sources(selected, fmt)

    # Шаг 2: LLM только исправляет пунктуацию/порядок, строго без добавления данных
    ...
    try:
        ...
        pass  # если проверка упала — оставляем детерминированный результат
...
3. Если в данных ошибка и ты знаешь правильный вариант — исправь и пометь ✏️ с пояснением.
...
ВАЖНО: Не придумывай альтернативы — только те источники, в реальном существовании \
которых ты абсолютно уверен (автор, название, издательство совпадают с реально изданной книгой).

Формат ответа (блок для каждого источника):
✅ [источник] — данные верны
✏️ [исправленный источник] — исправлено: [что именно]
⚠️ Не найдено: «[введённый текст]»
   Похожие реальные источники:
